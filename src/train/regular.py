...
def train(train_data, val_data, model, args, DA_data=None):
    ...
    train_gen = ParallelSampler(train_data, args, args.train_episodes, DA_data['train'])
    train_gen_val = ParallelSampler(train_data, args, args.val_episodes, DA_data['train'])
    val_gen = ParallelSampler(val_data, args, args.val_episodes, DA_data['val'])
...
def test(test_data, model, args, num_episodes, verbose=True, sampled_tasks=None, DA_data=None, test_mode=None):
    ...
    if sampled_tasks is None:
        sampled_tasks = ParallelSampler(test_data, args,
                                        num_episodes, DA_data).get_epoch()
